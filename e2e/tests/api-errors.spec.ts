...
test.describe('API Error Handling', () => {
  test.describe('Price API errors', () => {
    test('app remains functional when price API returns 500', async ({ page, appPage }) => {
  ...
  test.describe('Supplier API errors', () => {
    ...
    test('handles supplier list failure gracefully', async ({ page, appPage }) => {
  ...
  test.describe('Company API errors', () => {
    test('handles company list failure gracefully', async ({ page, appPage, settingsDialogPage }) => {
      ...
      await settingsDialogPage.switchToElectricityTab();
      await expect(page.getByPlaceholder(/postal code/i)).toBeVisible();
    });
  });
  ...
  test.describe('Network recovery', () => {
    test('recovers when network is restored after failure', async ({ page, appPage }) => {
      ...
      await expect(page.getByText('stromligning')).toBeVisible({ timeout: 10000 });
    });
  });

  test.describe('Rate limiting', () => {
    test('app remains functional when rate limited', async ({ page, appPage }) => {
